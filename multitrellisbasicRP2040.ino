...
TrellisCallback blink(keyEvent evt){
  ...
  if(evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING)
    trellis.setPixelColor(evt.bit.NUM, Wheel(map(evt.bit.NUM, 0, X_DIM*Y_DIM, 0, 255/2))); //on rising
...
void setup() {
  ...
  for(int i=0; i<Y_DIM*X_DIM; i++){
      trellis.setPixelColor(i, Wheel(map(i, 0, X_DIM*Y_DIM, 0, 255/2))); //addressed with keynum
      trellis.show();
      delay(50);
  }
